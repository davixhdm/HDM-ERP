import { useState } from 'react';
import StockOverview from '../../components/inventory/StockOverview';
import StockMovements from '../../components/inventory/StockMovements';
import Warehouses from '../../components/inventory/Warehouses';
import StockTransfers from '../../components/inventory/StockTransfers';
import { Package, ArrowRightLeft, Building2, Truck } from 'lucide-react';

const tabs = [
  { key: 'stock', label: 'Stock', icon: Package },
  { key: 'movements', label: 'Movements', icon: ArrowRightLeft },
  { key: 'warehouses', label: 'Warehouses', icon: Building2 },
  { key: 'transfers', label: 'Transfers', icon: Truck },
];

const InventoryPage = () => {
  const [active, setActive] = useState('stock');

  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Inventory</h1>
      <div className="flex gap-1 mb-6 border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
        {tabs.map(tab => (
          <button
            key={tab.key}
            onClick={() => setActive(tab.key)}
            className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
              active === tab.key ? 'border-primary-500 text-primary-500' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'
            }`}
          >
            <tab.icon size={14} />
            {tab.label}
          </button>
        ))}
      </div>
      {active === 'stock' && <StockOverview />}
      {active === 'movements' && <StockMovements />}
      {active === 'warehouses' && <Warehouses />}
      {active === 'transfers' && <StockTransfers />}
    </div>
  );
};

export default InventoryPage;