  ...
  return (
    <div className="fixed bottom-16 right-4 z-40 w-80 h-96 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col">
          ...
          <div className="text-center text-gray-400 dark:text-gray-500 mt-10">
            ...
            <div className="flex flex-wrap gap-1 mt-3 justify-center">
              {['Show revenue', 'Low stock items', 'Recent orders', 'Cash flow'].map(s => (
                <button key={s} onClick={() => sendQuery(s)} className="text-xs px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-full text-gray-600 dark:text-gray-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-500 transition-colors">
                  {s}
                </button>
              ))}
            </div>
          ...
          messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] px-3 py-2 rounded-lg whitespace-pre-wrap break-words ${
                msg.role === 'user' ? 'bg-primary-500 text-white rounded-br-none' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none'
              }`}>
                {msg.content}
              </div>
            </div>
      ...
      <div className="p-3 border-t border-gray-200 dark:border-gray-700 flex gap-2">
        ...
        />
        <button onClick={handleSend} disabled={loading || !input.trim()} className="p-2 bg-primary-500 text-white rounded-lg hover:bg-primary-600 disabled:opacity-50 transition-colors">
          <Send size={16} />
        </button>
      </div>
    </div>
